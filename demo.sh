...
prepare_rgb_wallet() {
    ...
    _trace "${BTCHOT[@]}" derive -s $der_scheme --testnet \
        --seed-password '' --account-password '' \
        "$WALLET_PATH/$wallet.seed" "$WALLET_PATH/$wallet.derive" >$TRACE_OUT
    descriptor="$(cat $TRACE_OUT | tail -2 | head -1)"
...
scenario_0() {  # default
    ...
    transfer_assets wallet_1/wallet_2  300/0     150  150/150  0 0 usdt         # spend both received allocations
    transfer_assets wallet_1/wallet_2  200/0     100  100/100  0 0 collectible  # CFA, spend received allocations
    transfer_assets wallet_2/wallet_0  150/1700  100   50/1800 1 0 usdt         # close loop, witness
    transfer_assets wallet_2/wallet_0  100/1800   50   50/1850 1 0 collectible  # CFA, close loop, witness
    transfer_assets wallet_0/wallet_1 1800/150    50 1750/200  0 0 usdt         # spend received back
