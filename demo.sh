...
transfer_create() {
    ...
    declare inputs=()
    local addr_send der_xpub filter inputs opret psbt_to utxos
    ...
    _trace $BDKI -n $NETWORK wallet -w "$SEND_WLT" \
        -d "${DESC_TYPE}($der_xpub)" create_tx --enable_rbf \
...
transfer_complete() {
    ...
    _subtit "(sender) accepting transfer"
